...
export default function CustomerHomePage() {
  ...
  const {
    ...
    selectedAddressId,
    customerLocation,
    hasRequestedCustomerLocation,
    setSelectedShopId,
    setSelectedAddressId,
    setCustomerLocation,
    setHasRequestedCustomerLocation,
  } = useUIStore();
  ...
  const productsQuery = useProductsQuery({ page: 1, limit: 40, shopId: selectedShopId });
  ...
  useEffect(() => {
    if (!selectedShopId && shopsQuery.data?.length) {
      setSelectedShopId(shopsQuery.data[0].id);
    }
  }, [selectedShopId, setSelectedShopId, shopsQuery.data]);

  useEffect(() => {
    if (!hasRequestedCustomerLocation) {
      setHasRequestedCustomerLocation(true);
  ...
  }, [hasRequestedCustomerLocation, setHasRequestedCustomerLocation, syncCurrentLocation]);
      ...
      <main className="mx-auto flex max-w-7xl flex-col gap-8 px-4 py-6 sm:px-6 lg:px-8">
            ...
            <div className="grid gap-4 p-8 md:grid-cols-[1.1fr_0.9fr]">
              <div className="space-y-4">
                ...
                <p className="max-w-lg text-sm text-white/90">
                  Browse nearby shops, find products from your selected area, and move straight
                  to checkout without hunting through admin-style screens.
                </p>
                <div className="flex flex-wrap gap-3">
                  <Link href={selectedShopId ? `/customer/shops/${selectedShopId}` : "/customer/shops"}>
              ...
              <div className="grid gap-3 self-end sm:grid-cols-2">
                <div className="rounded-[24px] bg-white/14 p-4 backdrop-blur">
                  ...
                  <p className="mt-2 text-xl font-bold">
                    {shopsQuery.data?.find((shop) => shop.id === selectedShopId)?.name ||
                      "Nearby store"}
                  </p>
                </div>
                <div className="rounded-[24px] bg-white/14 p-4 backdrop-blur">
          ...
          >
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
              {categoriesQuery.data?.slice(0, 12).map((category, index) => (
                ...
                >
                  <div
                    className="mb-4 flex h-20 items-center justify-center rounded-[20px] text-3xl"
                    style={{
                      background: [
                        "linear-gradient(135deg, rgba(31,157,85,0.18), rgba(31,157,85,0.04))",
                        "linear-gradient(135deg, rgba(240,140,0,0.18), rgba(240,140,0,0.04))",
                  ...
                  <p className="line-clamp-2 text-sm font-semibold text-[var(--color-secondary)]">
                    {category.name}
                  </p>
                </Link>
              ))}
        ...
        <section className="space-y-8">
          <DataState
            ...
            error={selectedShopId ? getErrorMessage(productsQuery.error, "") : null}
